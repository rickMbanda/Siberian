...
const MidtermExam = () => {
  ...
  const loadLockStatus = async () => {
    try {
      const status = await fetchLockStatus(selectedYear, selectedTerm, 'midterm');
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('Could not load class roster for midterm:', err);
        setStudents([]);
      } finally {
    ...
    setStudents(prev =>
      prev.map(student => {
        ...
        const updated = {
          ...student,
          [field]: value,
          examType: 'midterm', class: selectedClass,
