...
const OpenerExam = () => {
  ...
  const loadLockStatus = async () => {
    try {
      const status = await fetchLockStatus(selectedYear, selectedTerm, 'opener');
      setLockStatus(status);
    } catch (err) {
      console.error('Could not load lock status for opener exam:', err);
  ...
  const handleLock = async () => {
    ...
    try {
      await setLockConfig({ academicYear: selectedYear, term: selectedTerm, examType: 'opener', gracePeriodMinutes: Number(gracePeriod) || 0 });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('Could not load class roster for opener:', err);
        setStudents([]);
      } finally {
    ...
    setStudents(prev =>
      prev.map(student => {
        ...
        const updated = {
          ...student,
          [field]: value,
          examType: 'opener', class: selectedClass,
